...
def kakao_send_text_to_self(access_token: str, text: str, link_url: str) -> None:
    """발신자 본인의 카카오톡 '나와의 채팅'으로 메시지를 전송합니다."""
    template_object = {
        "object_type": "text",
        "text": text[:200],
        "link": {"web_url": link_url, "mobile_web_url": link_url},
...
def send_kakao(results: list[tuple[Site, list[Item], str | None]], total: int) -> tuple[int, int]:
    """
    선택된 카카오 친구들에게 메시지를 전송하고, KAKAO_SEND_TO_SELF=1인 경우
    발신자 본인의 '나와의 채팅'에도 함께 전송합니다.
    반환값은 (성공 수, 실패 수)입니다.
    """
    receiver_uuids: list[str] = []
    ...
    try:
        receiver_uuids = load_kakao_receiver_uuids()
        expected_count = len(receiver_uuids) + (1 if send_to_self else 0)
        ...
        # 1) 필요할 때만 발신자 본인에게 '나에게 보내기'
        if send_to_self:
            try:
                kakao_send_text_to_self(access_token, text, link_url)
                total_success += 1
                print("카카오톡 나에게 보내기 완료")
            except Exception as e:
                total_failure += 1
                print(f"카카오톡 나에게 보내기 실패: {e}")
        else:
            print("KAKAO_SEND_TO_SELF=0: 본인 카카오톡 발송은 건너뜁니다.")

        # 2) 선택된 친구들에게 보내기 (요청당 최대 5명)
        if not receiver_uuids:
            if send_to_self:
                print("선택된 카카오 친구 수신자가 없습니다. 본인에게만 전송했습니다.")
        else:
            batch_no = 0
            for batch in _chunks(receiver_uuids, 5):
                batch_no += 1
                try:
                    ...
                    total_failure += batch_failure
                    print(
                        f"카카오톡 친구 발송 배치 {batch_no}: "
                        f"성공 {batch_success}명 / 실패 {batch_failure}명"
                    )
                except Exception as e:
                    total_failure += len(batch)
                    print(f"카카오톡 친구 발송 배치 {batch_no} 실패: {e}")

        self_note = " / 본인 포함" if send_to_self else " / 본인 제외"
        print(
            f"카카오톡 발송 완료: 총 성공 {total_success}명 / "
            f"실패 {total_failure}명{self_note}"
        )
        return total_success, total_failure
